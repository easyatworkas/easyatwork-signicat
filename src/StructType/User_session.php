...
class User_session extends AbstractStructBase
{
     ...
     * Meta informations extracted from the WSDL
     * - maxOccurs: unbounded
     * @var \Easyatwork\Signicat\StructType\Session_task_info[]
     */
    public $session_task;
    /**
     ...
     * @param \Easyatwork\Signicat\StructType\Session_task_info[] $session_task
     */
     ...
     * Set language value
     * @param string $language
     * @return \Easyatwork\Signicat\StructType\User_session
     */
     ...
     * @return \Easyatwork\Signicat\StructType\Session_task_info[]|null
     */
    ...
    public function setSession_task(array $session_task = array())
    {
        foreach ($session_task as $user_sessionSession_taskItem) {
            ...
            if (!$user_sessionSession_taskItem instanceof \Easyatwork\Signicat\StructType\Session_task_info) {
                throw new \InvalidArgumentException(sprintf('The session_task property can only contain items of \Easyatwork\Signicat\StructType\Session_task_info, "%s" given', is_object($user_sessionSession_taskItem) ? get_class($user_sessionSession_taskItem) : gettype($user_sessionSession_taskItem)), __LINE__);
            }
        }
    ...
    /**
     */
    public function addToSession_task(\Easyatwork\Signicat\StructType\Session_task_info $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Easyatwork\Signicat\StructType\Session_task_info) {
            throw new \InvalidArgumentException(sprintf('The session_task property can only contain items of \Easyatwork\Signicat\StructType\Session_task_info, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
