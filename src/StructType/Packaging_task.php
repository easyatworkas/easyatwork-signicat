...
class Packaging_task extends AbstractStructBase
{
     ...
     * Meta informations extracted from the WSDL
     * - maxOccurs: unbounded
     * @var \Easyatwork\Signicat\StructType\Packaging_task_document[]
     */
     ...
     * @param string $method
     * @param \Easyatwork\Signicat\StructType\Notification[] $packaging_notification
     * @param \Easyatwork\Signicat\StructType\Packaging_task_document[] $packaging_task_document
     ...
     * @param string $packaging_task_id
     * @return \Easyatwork\Signicat\StructType\Packaging_task
     */
    ...
    public function setPackaging_notification(array $packaging_notification = array())
    {
        foreach ($packaging_notification as $packaging_taskPackaging_notificationItem) {
            ...
            if (!$packaging_taskPackaging_notificationItem instanceof \Easyatwork\Signicat\StructType\Notification) {
                throw new \InvalidArgumentException(sprintf('The packaging_notification property can only contain items of \Easyatwork\Signicat\StructType\Notification, "%s" given', is_object($packaging_taskPackaging_notificationItem) ? get_class($packaging_taskPackaging_notificationItem) : gettype($packaging_taskPackaging_notificationItem)), __LINE__);
            }
        }
    ...
    /**
     */
    public function addToPackaging_notification(\Easyatwork\Signicat\StructType\Notification $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Easyatwork\Signicat\StructType\Notification) {
            throw new \InvalidArgumentException(sprintf('The packaging_notification property can only contain items of \Easyatwork\Signicat\StructType\Notification, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
     ...
     * @return \Easyatwork\Signicat\StructType\Packaging_task_document[]|null
     */
    ...
    public function setPackaging_task_document(array $packaging_task_document = array())
    {
        foreach ($packaging_task_document as $packaging_taskPackaging_task_documentItem) {
            // validation for constraint: itemType
            if (!$packaging_taskPackaging_task_documentItem instanceof \Easyatwork\Signicat\StructType\Packaging_task_document) {
                throw new \InvalidArgumentException(sprintf('The packaging_task_document property can only contain items of \Easyatwork\Signicat\StructType\Packaging_task_document, "%s" given', is_object($packaging_taskPackaging_task_documentItem) ? get_class($packaging_taskPackaging_task_documentItem) : gettype($packaging_taskPackaging_task_documentItem)), __LINE__);
            }
        }
    ...
    /**
     */
    public function addToPackaging_task_document(\Easyatwork\Signicat\StructType\Packaging_task_document $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Easyatwork\Signicat\StructType\Packaging_task_document) {
            throw new \InvalidArgumentException(sprintf('The packaging_task_document property can only contain items of \Easyatwork\Signicat\StructType\Packaging_task_document, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
    ...
    public function setSend_to_archive($send_to_archive = null)
    {
        ...
        if (!is_null($send_to_archive) && !is_bool($send_to_archive)) {
            throw new \InvalidArgumentException(sprintf('Invalid value, please provide a bool, "%s" given', gettype($send_to_archive)), __LINE__);
        }
