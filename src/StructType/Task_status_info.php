...
class Task_status_info extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \Easyatwork\Signicat\StructType\Document_status[]
     */
     ...
     * @param \Easyatwork\Signicat\StructType\Document_status[] $document_status
     */
     ...
     * @param string $request_id
     * @return \Easyatwork\Signicat\StructType\Task_status_info
     */
    ...
    public function setTask_status($task_status = null)
    {
        ...
        if (!\Easyatwork\Signicat\EnumType\Task_status::valueIsValid($task_status)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $task_status, implode(', ', \Easyatwork\Signicat\EnumType\Task_status::getValidValues())), __LINE__);
        }
     ...
     * @return \Easyatwork\Signicat\StructType\Document_status[]|null
     */
    ...
    public function setDocument_status(array $document_status = array())
    {
        foreach ($document_status as $task_status_infoDocument_statusItem) {
            ...
            if (!$task_status_infoDocument_statusItem instanceof \Easyatwork\Signicat\StructType\Document_status) {
                throw new \InvalidArgumentException(sprintf('The document_status property can only contain items of \Easyatwork\Signicat\StructType\Document_status, "%s" given', is_object($task_status_infoDocument_statusItem) ? get_class($task_status_infoDocument_statusItem) : gettype($task_status_infoDocument_statusItem)), __LINE__);
            }
        }
    ...
    /**
     */
    public function addToDocument_status(\Easyatwork\Signicat\StructType\Document_status $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Easyatwork\Signicat\StructType\Document_status) {
            throw new \InvalidArgumentException(sprintf('The document_status property can only contain items of \Easyatwork\Signicat\StructType\Document_status, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
